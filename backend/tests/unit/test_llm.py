...
async def test_generate_digest_propagates_ollama_error(db):
    ...
    db.add(Article(
        ...
        excerpt="Excerpt", fetched_at=datetime.utcnow(),
    ))
    await db.commit()
...
async def test_generate_digest_topic_filter(db):
    ...
    db.add(Article(
        ...
        excerpt="Tech stuff", fetched_at=datetime.utcnow(),
    ))
    db.add(Article(
        feed_id=feed_out.id, title="Other article", url="https://other.com/a1",
        excerpt="Other stuff", fetched_at=datetime.utcnow(),
    ))
    await db.commit()
...
async def test_summarize_article_falls_back_to_excerpt(db):
    ...
    article = Article(
        ...
        excerpt="A brief excerpt.", fetched_at=datetime.utcnow(),
    )
    db.add(article)
    await db.commit()
...
async def test_summarize_article_returns_none_when_no_content(db):
    ...
    article = Article(
        ...
        excerpt=None, fetched_at=datetime.utcnow(),
    )
    db.add(article)
    await db.commit()
